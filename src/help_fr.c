...
InitInstructions()
{
  ...
  if (err == errNone) 
  {
...

    {
      ...
      x = 4;
      StrCopy(str,
"Liberty supporte les jeux créés pour la console GameBoy\231.");
      ptrStr = str;
	...
	ptrStr += count;
      ...
      x = 4;
      StrCopy(str, 
"Choississez le jeu GameBoy auquel vous souhaitez jouer dans la liste des ROMs installées.");
      ptrStr = str;
      while (StrLen(ptrStr) != 0) 
      {
        UInt8 count = FntWordWrap(ptrStr, rect.extent.x-x);
	...
	DmReleaseResource(bitmapHandle);
      ...
      x = 4;
      StrCopy(str,
"Par le biais du menu, vous pouvez commencer l'émulation ou \
supprimer une ROM de votre machine.");
	...
	ptrStr += count;
      ...
      x = 4;
      StrCopy(str,
"Chaque partie peut être chargée ou sauvée à nimporte quel moment et \
possède sa propre configuration des options (voir plus bas).");
      ptrStr = str;
      while (StrLen(ptrStr) != 0) 
      {
        UInt8 count = FntWordWrap(ptrStr, rect.extent.x-x);
	...
	ptrStr += count;
      ...
      x = 4;
      StrCopy(str,
"Définissez les touches à votre convenance ou utilisez la zone grafitti \
comme indiqué ci-dessous :");
	...
	DmReleaseResource(bitmapHandle);
      ...
      x = 4;
      StrCopy(str,
"L'option de raffraîchissement détermine le nombre de fois qu'une \
image d'un jeu GameBoy copied to the screen.");
	...
	ptrStr += count;
      ...
      x = 4;
      StrCopy(str,
"Ajustez l'affichage à votre convenance. Si vous possédez une machine couleur \
alors vous pouvez même personnaliser les couleurs ! Quel plaisir pour les yeux !");
      ptrStr = str;
      while (StrLen(ptrStr) != 0) 
      {
        UInt8 count = FntWordWrap(ptrStr, rect.extent.x-x);
